...
function validateFields(
    ...
    if (isSignup && fullName.trim().length === 0) errors.fullName = "Full name is required";
...
const RegisterForm = ({ mode }: RegisterFormProps) => {
    ...
    const canSubmit = isLogin ? hasNoErrors && !loading : hasNoErrors && agreedToTerms && !loading;
            ...
            <form onSubmit={handleSubmit} noValidate>
                {isSignup && (
                    <div className="mb-3">
                        ...
                        {showFieldError("fullName") && (
                            <div className="text-danger mt-1" style={{ fontSize: '0.75rem' }}>{showFieldError("fullName")}</div>
                        )}
                    </div>
                )}
                ...
                <div className="mb-3">
                    ...
                    {showFieldError("email") && (
                        <div className="text-danger mt-1" style={{ fontSize: '0.75rem' }}>{showFieldError("email")}</div>
                    )}
                </div>
                ...
                <div className="mb-3">
                    ...
                    {showFieldError("password") ? (
                        <div className="text-danger mt-1" style={{ fontSize: '0.75rem' }}>{showFieldError("password")}</div>
                    ) : isSignup && password.length > 0 ? (
                        <div className="mt-2">
                            <div className="d-flex gap-1">
                                {[0, 1, 2].map((i) => (
                                    <div key={i} className={`flex-grow-1 strength-bar rounded-pill ${i < strength ? strengthBarClass(strength) : "strength-empty"}`} />
                                ))}
                            </div>
                            <div className="d-flex justify-content-between mt-1">
                                <small className="text-secondary" style={{ fontSize: '0.6875rem' }}>Use 8+ chars, a number & symbol</small>
                                {strength > 0 && (
                                    <small className={`fw-semibold ${strengthTextClass(strength)}`} style={{ fontSize: '0.6875rem' }}>
                    ...
                    <div className="mb-3">
                        ...
                        <div className="auth-input-group">
                            ...
                            <input
                                ...
                                style={{ paddingRight: '2.75rem' }}
                            />
                            <button type="button" className="auth-eye-btn" onClick={() => setShowConfirmPassword(v => !v)}>
                                <i className={`bi ${showConfirmPassword ? "bi-eye-slash" : "bi-eye"}`}></i>
                            </button>
                        </div>
                        {showFieldError("confirmPassword") && (
                            <div className="text-danger mt-1" style={{ fontSize: '0.75rem' }}>{showFieldError("confirmPassword")}</div>
                        )}
                    </div>
                )}
                ...
                {isSignup && (
                    <div className="mb-3 d-flex align-items-start gap-2">
                        ...
                        />
                        <label htmlFor="terms" className="text-secondary" style={{ fontSize: '0.8125rem', cursor: 'pointer', lineHeight: 1.5 }}>

...
const PortfolioStep: React.FC = () => {
  ...
  const isValid =
    ...
    formData.portfolio.every(
      (p) => p.title.trim() !== "" && p.images.length >= 1,
    ) &&
    ...
    try {
      for (const file of Array.from(files)) {
        if (file.size > 2 * 1024 * 1024) {
          toast.warning(
            `File "${file.name}" exceeds 2MB limit and was skipped.`,
          );
          continue;
        }
      ...
      <div className="card border-0 shadow-sm rounded-4 p-4 p-md-5">
        <div className="text-center mb-4">
          ...
          <p className="text-secondary small">
            Clients are more likely to hire providers with real project
            examples.
          </p>
        </div>
        ...
        <div className="mb-4">
          {formData.portfolio.map((project, index) => (
            <div
              key={project.id}
              className="card border shadow-sm rounded-4 mb-4 position-relative"
            >
                ...
                <div>
                  ...
                  >
                    {uploadingProjects[project.id] ? (
                      ...
                      <>
                        <i className="bi bi-image text-primary"></i> Upload
                        Images
                      </>
                    )}
                  </button>

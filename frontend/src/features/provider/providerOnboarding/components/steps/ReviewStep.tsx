            ...
            <div className="card border-0 shadow-sm rounded-4 p-4 p-md-5 mb-4">
                ...
                <div className="mb-4 bg-light rounded-4 p-4 border position-relative">
                    ...
                    <div className="d-flex align-items-center gap-4">
                        {formData.profileImage ? (
                            <img src={formData.profileImage} alt="Profile" className="rounded-circle object-fit-cover shadow-sm border" style={{ width: "90px", height: "90px" }} />
                        ) : (
                            <div className="rounded-circle bg-white d-flex align-items-center justify-content-center shadow-sm border" style={{ width: "90px", height: "90px" }}>
                                <i className="bi bi-person text-secondary fs-1"></i>
                            </div>
                        )}
                        <div>
                            <h5 className="fw-bold mb-1">{formData.headline || "Professional Service Provider"}</h5>
                            <p className="text-muted small mb-2">{formData.yearsOfExperience} Years of Experience</p>
                            <p className="text-muted small mb-0"><i className="bi bi-telephone-fill me-2"></i> {formData.phone.replace(/.(?=.{4})/g, '*') || "N/A"}</p>
                        </div>
                    </div>

                    {formData.about && (
                        <div className="mt-3 pt-3 border-top">
                            <p className="text-secondary small mb-0 lh-sm">"{formData.about}"</p>
                        </div>
                    )}
                </div>
                        ...
                        <div className="col-sm-6">
                            ...
                            <div className="d-flex flex-wrap gap-1">
                                {formData.skills.map(skill => (
                                    <span key={skill.id} className="badge bg-primary bg-opacity-10 text-primary rounded-pill fw-normal">
                                        {skill.name}
                                    </span>
                                ))}
                                {formData.skills.length === 0 && <span className="text-danger small">Required</span>}
                            </div>
                        </div>
                        <div className="col-sm-6 text-sm-end">
                            <p className="text-muted small fw-bold mb-1">Hourly Rate</p>
                            <p className="fw-bold fs-5 mb-0 text-dark">₹{formData.hourlyRate}<span className="text-muted fw-normal fs-6">/hr</span></p>
                        </div>
                    </div>

                    <div className="mt-3 pt-3 border-top d-flex align-items-center gap-2">
                        <i className="bi bi-geo-alt-fill text-primary"></i>
                        <span className="fw-medium text-dark">{formData.location?.name || <span className="text-danger">Location Required</span>}</span>
                    </div>
                </div>
                ...
                <div className="mb-4 bg-light rounded-4 p-4 border">
                    ...
                    {formData.portfolio.length === 0 ? (
                        <p className="text-danger small mb-0">At least 1 project is required.</p>
                    ) : (
                        <div className="d-flex flex-column gap-3">
                            {formData.portfolio.map((project, idx) => (
                                <div key={project.id} className={idx !== formData.portfolio.length - 1 ? "border-bottom pb-3" : ""}>
                                    <h6 className="fw-bold mb-1">{project.title}</h6>
                                    {project.description && <p className="text-secondary small mb-2 lh-sm">{project.description}</p>}
                                    <div className="d-flex gap-2">
                                        {project.images.slice(0, 3).map((img, imgIdx) => (
                                            <img key={imgIdx} src={img} alt="Preview" className="rounded-3 border object-fit-cover shadow-sm" style={{ width: "60px", height: "60px" }} />
                                        ))}
                                        {project.images.length > 3 && (
                                            <div className="rounded-3 border bg-white text-secondary d-flex align-items-center justify-content-center fw-bold small shadow-sm" style={{ width: "60px", height: "60px" }}>
                                                +{project.images.length - 3}
                                            </div>
                                        )}
                                    </div>
                ...
                <div className="bg-primary bg-opacity-10 rounded-4 p-4 border border-primary border-opacity-25 mt-4">
                    ...
                    <div className="form-check mb-3">
                        ...
                        />
                        <label className="form-check-label text-dark small" htmlFor="accuracyCheck">
                            I confirm that the information provided is accurate and truthfully represents my professional background.
                        </label>
                    </div>
                    ...
                    <div className="form-check">
                        ...
                        <label className="form-check-label text-dark small" htmlFor="termsCheck">
                            I agree to QuickWork's service guidelines and understand that my profile is subject to approval.
                        </label>
                    </div>
                </div>

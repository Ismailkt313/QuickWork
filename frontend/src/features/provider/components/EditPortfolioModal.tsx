...
import { RiCloseLine, RiSaveLine, RiDeleteBin7Line, RiImageAddLine, RiLoader4Line } from 'react-icons/ri';
...
const EditPortfolioModal: React.FC<EditPortfolioModalProps> = ({ 
  ...
  const validate = () => {
    ...
    if (formData.images.length === 0) newErrors.images = "At least one project image is required";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updateProviderProfile({ portfolio: updatedPortfolio });
      if (response.success) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const updatedPortfolio = portfolio.filter(item => item.title !== itemToEdit.title);
    ...
    <div className="qw-modal-backdrop animate__animated animate__fadeIn">
      <div className="qw-modal-content edit-portfolio p-4 animate__animated animate__zoomIn">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 className="qw-h2 m-0 text-primary">{itemToEdit ? 'Edit Project' : 'Add New Project'}</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="row g-3">
            ...
            <div className="col-12">
              <label className="form-label font-md fw-600">Project Description</label>
              <textarea 
            ...
            <div className="col-12">
              <label className="form-label font-md fw-600">Project Images</label>
              <div className="qw-upload-zone mb-3" onClick={() => fileInputRef.current?.click()}>
                <input 
                  type="file" 
                  ref={fileInputRef} 
                ...
                <div className="d-flex flex-column align-items-center gap-2">
                  ...
                  <p className="small fw-600 mb-0">Click to upload project photo</p>
                  <p className="text-muted smaller mb-0">JPEG, PNG up to 2MB</p>
                </div>
              </div>
              
              {errors.images && <div className="text-danger small mb-3">{errors.images}</div>}

              <div className="qw-image-preview-grid">
                {formData.images.map((url: string, index: number) => (
                  <div key={index} className="qw-image-preview">
                    <img src={url} alt={`Preview ${index}`} />
                    <button type="button" className="qw-remove-image-btn" onClick={() => removeImage(index)}>
                      <RiCloseLine />
                    </button>
                  </div>
                ))}
                {formData.images.length === 0 && (
                  <div className="qw-image-placeholder">
                    <RiImageAddLine size={40} className="text-muted opacity-25" />
          ...
          <div className="d-flex gap-3 mt-5 pt-3 border-top">
            {itemToEdit && (
              <button type="button" className="btn btn-outline-danger px-4 rounded-pill d-flex align-items-center gap-2" onClick={handleDelete} disabled={loading}>
            ...
            <div className="ms-auto d-flex gap-3">
              ...
              <button type="submit" className="btn btn-primary rounded-pill px-5 d-flex align-items-center justify-content-center gap-2" disabled={loading}>

...
const AssignmentDetailPage: React.FC = () => {
  ...
  const handleCancelAssignment = async (notes: string) => {
    try {
      setActionLoading(true);
      const response = await cancelAssignmentByProvider(assignmentId, notes);
      if (response.success) {
  ...
  const handleMessageProvider = (providerId: string, providerName: string) => {
    navigate(
      `/provider/messages?userId=${providerId}&name=${encodeURIComponent(providerName)}`,
    );
  };
    ...
    <div className="py-4 px-3 px-lg-5 animate-in">
      ...
      <div className="row g-4">
        <div className="col-12 col-xl-8">
          <div className="bg-white p-5 rounded-5 shadow-sm border border-f1f5f9 mb-4 overflow-hidden position-relative">
            ...
            <div className="d-flex flex-wrap gap-2 mb-3">
              {job.isUrgent && (
                <span className="badge bg-danger-subtle text-danger px-3 py-2 rounded-2 fw-bold">
                  Urgent
                </span>
              )}
              <span className="badge bg-primary-subtle text-primary px-3 py-2 rounded-2 fw-bold">
            ...
            {workStatus === "completed" && assignment.proof && (
              <div className="p-4 bg-muted-light rounded-4 border border-f1f5f9">
                ...
                <p className="text-muted mb-3 italic">
                  "{assignment.proofDescription || "No description provided."}"
                </p>
                  ...
                  <div key={worker.id} className="col-12 col-md-6">
                    <div className="p-3 rounded-4 border bg-light d-flex align-items-center gap-3">
                      <div className="position-relative">
                        ...
                        <div
                          ...
                          style={{
                            ...
                            height: 12,
                            background:
                              worker.workStatus === "completed"
                                ? "#10b981"
                                : worker.workStatus === "in_progress"
                                  ? "#f59e0b"
                                  : "#6366f1",
                          }}
                        />
                      ...
                      <div className="d-flex align-items-center gap-2">
                        ...
                        <button
                          className="btn btn-white btn-sm rounded-circle shadow-sm border p-2"
                          onClick={() =>
                            handleMessageProvider(worker.userId, worker.name)
                          }
                          title="Message Provider"
                        >
                          <RiMessage2Line size={16} className="text-primary" />
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="col-12 col-xl-4">
          <div
            className="bg-white p-4 rounded-5 shadow-sm border border-f1f5f9 mb-4 sticky-top"
            style={{ top: 20 }}
          >
            <h5
            ...
            <div className="p-4 bg-light rounded-4 mb-4 d-flex align-items-center gap-3">
              ...
              <div>
                <p className="mb-0 text-muted small fw-bold text-uppercase">
                  Client
                </p>
            ...
            <div className="d-grid gap-3">
                ...
                <>
                  <div className="p-3 bg-blue-50 rounded-4 border border-blue-100 mb-2">
                    ...
                    <p className="small text-muted mb-0">
                      Commence the work to let the client know you've started.
                    </p>
                  </div>
                  <button
              ...
              {workStatus === "in_progress" && (
                <>
                  <div className="p-3 bg-orange-50 rounded-4 border border-orange-100 mb-2">
                    ...
                    <p className="small text-muted mb-0">
                      Provide proof of work after finishing the tasks to
                      complete the job.
                    </p>
                  </div>
                  <button
              ...
              {workStatus === "completed" && (
                ...
                {workStatus !== "completed" && workStatus !== "cancelled" && (
                  <button
                    className="btn btn-outline-danger py-3 rounded-4 fw-bold d-flex align-items-center justify-content-center gap-2 mt-2"

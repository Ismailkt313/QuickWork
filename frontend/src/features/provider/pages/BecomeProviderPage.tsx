    ...
    useEffect(() => {
        const checkStatus = async () => {
            try {
                ...
                if (response.success && response.data?.verificationStatus === 'rejected') {
                    setIsRejected(true);
                    setRejectionReason(response.data.rejectionReason || "");
                }
            } catch (error) {
                // If 404, it means no profile exists yet, which is fine for new onboarding
                console.log("No existing provider profile or error fetching profile:", error);
            } finally {
        ...
        return (
            <div className="min-vh-100 d-flex align-items-center justify-content-center bg-white">
                <div className="spinner-border text-primary" role="status">
                    <span className="visually-hidden">Checking application status...</span>
                </div>
            </div>
        );
    }

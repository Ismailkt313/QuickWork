...
const RequestsPage: React.FC = () => {
    ...
    const [actionError, setActionError] = useState<{isOpen: boolean, title: string, message: string}>({
        isOpen: false,
        title: '',
        message: ''
    });
    ...
    return (
        <div className="container-fluid py-4 px-lg-5" style={{ backgroundColor: '#f8fafc', minHeight: '100vh' }}>
            ...
            <div className="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3">
                <div>
                    <h2 className="mb-1 fw-bold" style={{ color: '#0f172a', fontFamily: 'Syne, sans-serif', letterSpacing: '-0.5px' }}>Direct Invitations</h2>
                    <p className="text-muted mb-0" style={{ fontSize: '14.5px' }}>Manage jobs where clients have specifically requested your services.</p>
                </div>
                <div className="d-flex align-items-center gap-4 bg-white p-2 rounded-4 shadow-sm border border-f1f5f9">
                    <div className="text-center px-3 border-end">
                        <div className="fw-bold text-primary" style={{ fontSize: '18px' }}>{pendingCount}</div>
                        <div className="text-muted" style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' }}>Pending</div>
                    </div>
                    <div className="text-center px-3">
                        <div className="fw-bold text-success" style={{ fontSize: '18px' }}>{acceptedCount}</div>
                        <div className="text-muted" style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' }}>Accepted</div>
                    </div>
                </div>
            </div>
            ...
            <div className="d-flex align-items-center gap-2 mb-4 overflow-x-auto pb-2">
                {[
                    ...
                    { id: 'pending', label: 'Pending', icon: <RiMailOpenLine />, count: pendingCount },
                    ...
                    >
                        ...
                        {tab.count !== undefined && tab.count > 0 && (
                            <span className={`badge rounded-pill ms-1 ${filter === tab.id ? 'bg-white text-primary' : 'bg-primary-subtle text-primary'}`} style={{ fontSize: '10px' }}>
                                {tab.count}
                            </span>
                        )}
                    </button>
                ...
                <div className="col-12 col-xl-10">
                    {loading ? (
                        <div className="d-flex flex-column align-items-center justify-content-center py-5">
                            ...
                            <p className="text-muted fw-semibold">Discovering your invitations...</p>
                        </div>
                    ) : filteredRequests.length === 0 ? (
                        <div className="text-center py-5 px-4 bg-white rounded-5 border border-dashed border-2 border-slate-200 mt-4" style={{ borderStyle: 'dashed' }}>
                            <div className="mb-4 d-inline-flex align-items-center justify-content-center" style={{ width: 84, height: 84, borderRadius: 24, background: '#f1f5f9', color: '#94a3b8' }}>
                                <RiInboxLine size={42} />
                            </div>
                            <h4 className="fw-bold text-dark" style={{ fontFamily: 'Syne, sans-serif' }}>No {filter !== 'all' ? filter : ''} requests found</h4>
                            <p className="text-muted mx-auto" style={{ maxWidth: 360, fontSize: '14.5px' }}>
                                When clients hire you directly from your profile, their invitations will appear here for you to accept or reject.
                            </p>
                            <button className="btn btn-outline-primary px-4 py-2-5 rounded-3 fw-bold mt-2" onClick={fetchRequests}>
                                Refresh Page
                            </button>
                        </div>
                    ) : (

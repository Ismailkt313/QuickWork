...
const getIcon = (name: string, slug?: string) => {
  ...
  return Object.entries(SKILL_ICONS).find(([k]) => key.includes(k))?.[1] ?? '🛠️';
};
...
const AllServicesPage: React.FC = () => {
  ...
  const { locations, selectedLocation, selectLocation, clearLocation } = useLandingData();
    ...
    >
      <div style={{ background: 'linear-gradient(135deg,#0f172a,#1e3a5f)', padding: '48px 0 40px' }}>
        <div className="container">
          ...
          <h1 style={{ fontSize: 'clamp(1.6rem,4vw,2.4rem)', fontWeight: 800, color: '#f1f5f9', letterSpacing: '-0.02em', margin: 0 }}>
            All Services
          </h1>
          <p style={{ color: '#94a3b8', marginTop: 8, fontSize: 15 }}>
            {skills.length > 0 ? `${skills.length} services available` : 'Browse all available services'}
        ...
        <div className="container d-flex align-items-center gap-3 flex-wrap">
          ...
          <button
            ...
            style={{
              ...
              border: '1.5px solid', borderColor: selectedLocation ? '#bfdbfe' : '#e2e8f0',
          ...
          {selectedLocation && (
            <button
              ...
              style={{ padding: '6px 12px', borderRadius: 20, border: '1.5px solid #e2e8f0', background: '#fff', fontSize: 12.5, fontWeight: 600, color: '#94a3b8', cursor: 'pointer' }}
            >
              ✕ Clear
            </button>
          )}
          <span style={{ marginLeft: 'auto', fontSize: 13, color: '#94a3b8' }}>
            {!loading && `${skills.length} result${skills.length !== 1 ? 's' : ''}`}
          </span>
        </div>
      </div>
          ...
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 16 }}>
              {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} style={{ background: '#f1f5f9', borderRadius: 14, padding: '24px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 56, height: 56, borderRadius: 12, background: '#e2e8f0' }} />
            ...
            <div style={{ textAlign: 'center', padding: '80px 0', color: '#94a3b8' }}>
              ...
              <h3 style={{ fontWeight: 700, color: '#475569', marginBottom: 8 }}>No services found</h3>
              <p style={{ fontSize: 14 }}>
                {search ? `No results for "${search}". Try a different keyword.` : 'No services available right now.'}
              </p>
              {search && (
                <button onClick={() => setSearch('')} style={{ marginTop: 16, padding: '10px 24px', borderRadius: 10, border: 'none', background: '#3b82f6', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                  Clear Search
                </button>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 16 }}>
              {skills.map(skill => (
                <div
                  ...
                  style={{
                    background: '#fff', borderRadius: 14, padding: '24px 12px',
                    textAlign: 'center', cursor: 'pointer',
                    border: '1.5px solid #f1f5f9', transition: 'all 0.22s ease',
                ...
                >
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#1e293b', textTransform: 'capitalize', lineHeight: 1.3 }}>
                    {skill.name}
                  </span>
                </div>
              ))}
